...
An Alula controller is a struct, and a route is one of its methods. A service
...
- **The `:id` path parameter is an ordinary argument.** The route binds it
  to `id: UUID`, so a test passes a `UUID` and never spells the path.
  `RequestContext.mock(...)` supplies the rest with no transport behind it:
  headers and a body when you need them, nothing when you don't.
...
`Controller.alulaRoutes(_:)` is generated by `@Controller` and returns every
...
`AlulaDataTesting` ships an `InMemoryDataSource` for testing against the
