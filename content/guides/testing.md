...
Alula testing has four tiers, and the useful question is always *which one does
this belong in*:
...
struct that takes its collaborators. So most Alula tests are ordinary Swift —
...
route the controller declares, built from the one closure you hand it. (Each
route also has its own `_alulaRoute_<method>_<n>` factory, for the rarer case
of registering a *subset* deliberately — to isolate a single middleware lane,
say.) Keep this tier small: a handful of representative paths, not one per
handler. The unit tests above are where logic is exercised; these prove the
plumbing once.
...
different engine would test a different engine: Alula's SQL uses `DISTINCT ON`,
...
One thing that is *not* the answer: `AlulaDataTesting`'s `InMemoryDataSource`
...
func joinRejected() async throws {
    ...
    #expect(error == Envelope(ref: "9", topic: "room:locked", event: "alula:error",
